        ...
        <div className="flex gap-6">
          ...
          <aside className="hidden md:block w-48 shrink-0">
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden sticky top-20">
              ...
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`w-full text-right px-4 py-3 text-sm transition-all border-b border-gray-50 last:border-0
                    ${activeCategory === cat ? "bg-green-50 text-green-700 font-bold" : "text-gray-600 hover:bg-gray-50"}`}>
                  {cat}
                </button>
              ))}
            </div>
